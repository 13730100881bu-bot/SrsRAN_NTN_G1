...
- 最终方案采用 46 个轨道面、每面 65 星，共 2,990 星，比历史 3,528 星展示对照减少 538 星。
- 一天内每 120 秒一次的 720 个检查时刻均未发现覆盖空缺，并且每个检查时刻都完成了全网唯一分配；实际负责峰值为 87 个一级波位/星。
- 3,528 星不再作为目标规模，仅作为历史展示对照。当前互动页面仍用它展示轨道、负载和日历；2,990 星的新身份表与运行输入将在下一步生成。
...
| 3,528 星历史展示对照 | 一天固定步长检查未发现覆盖空缺；单星最多可见约 209 个一级波位 | 不再作为目标规模；“可见”也不是“实际负责” |

...
| NTN C-RNTI lease pools | CU-CP implements with exact DU/MAC exception | Task-specific | CU-CP is authoritative in NTN mode; DU/MAC consume pre-distributed leases for RAR and report state. With NTN inactive, terrestrial selection and default outcomes remain unchanged; shared synchronization overhead is not a performance-equivalence claim. |

...
> 本文把全球地固两级波位、星载双小区和轨道搜索连接起来，范围是管理中心/Web 的离线规划输入。CU-CP/DU 软件闭环见 [星载双小区计划输入、激活与恢复](./ntn_onboard_position_plan.md)，本文不把离线规划证据写成运行态或 RF 证据。旧中国 2,620 L1 / 18,208 L2 和 `53°:720/30/1` 只作为历史样例。
...
| 卫星短号 | `Pxx-Syy` | `P07-S41` | 当前 registry 的 opaque exact-match 键，不编码 NCI |

...
> 本文定义全球目标的轨道搜索输入和验收方法。当前只修改中文方案与 Web 离线展示，不修改 CU-CP、F1AP、DU、MAC、PHY、RU、C++ API、协议容器或运行配置。`Walker Delta 60°:3528/42/0` 只是搜索 seed；在真实 7 天事件驱动精确审计完成前，`selectedScenario` 为空，不存在“已通过”的最终星座。
...
- 卫星规划短号建议使用 `P01-S001`～`P42-S084`；短号、轨道面、槽位和 epoch 是独立属性。

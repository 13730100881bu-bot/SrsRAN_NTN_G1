...
| NTNPLAN-001 | Historical state before NTNPLAN-002: separated complete L1 visibility from unique service assignment in the offline planner and added a reproducible sampled comparison with a verified catalog content hash. At that point the Web displayed 3,528 satellites and treated 2,990 as an unselected coarse candidate. The 256/128 limits apply only to actual assignment, PRACH is planned per assigned L1, and current CU-CP still cannot consume complete visibility and actual assignment as two independent sets. NTNPLAN-002 supersedes only the constellation decision status, not these evidence boundaries. | `utils/ntn/constellation_screen.*`, `utils/ntn/scenarios/global_constellation_screen_v1*`, `docs/ntn_orbit_constellation_plan.md`, `docs/ntn_beam_hopping_access_plan.md`, Web planner source/tests and the separate `gh-pages` deployment. |

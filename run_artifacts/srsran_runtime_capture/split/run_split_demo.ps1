...
wsl.exe -e bash -lc "timeout 120s bash -lc 'pkill -TERM srsue 2>/dev/null || true; pkill -TERM srsdu 2>/dev/null || true; pkill -TERM srscuup 2>/dev/null || true; pkill -TERM srscucp 2>/dev/null || true; pkill -TERM tcpdump 2>/dev/null || true; sleep 2; if ss -S -l -n 2>/dev/null | grep -q 127.0.0.1:38412; then echo Open5GS AMF listener already ready; exit 0; fi; timeout 30s systemctl start mongod open5gs-nrfd || true; for i in {1..30}; do ss -l -n 2>/dev/null | grep -q 127.0.0.10:7777 && break; sleep 1; done; timeout 60s systemctl start open5gs-udrd open5gs-udmd open5gs-ausfd open5gs-amfd open5gs-smfd open5gs-upfd open5gs-pcfd open5gs-nssfd open5gs-bsfd || true; for i in {1..60}; do ss -S -l -n 2>/dev/null | grep -q 127.0.0.1:38412 && { echo Open5GS AMF listener ready; exit 0; }; sleep 1; done; echo Open5GS AMF listener did not become ready >&2; systemctl is-active mongod open5gs-amfd open5gs-smfd open5gs-upfd open5gs-nrfd open5gs-ausfd open5gs-udmd open5gs-udrd open5gs-pcfd open5gs-nssfd open5gs-bsfd || true; ss -S -l -n 2>/dev/null | grep 38412 || true; exit 1'"
if ($LASTEXITCODE -ne 0) {
  throw "Open5GS AMF is not ready at 127.0.0.1:38412."
}

...
setsid -f bash -c "$(declare -f cu_cp_stdin_pipeline); cu_cp_stdin_pipeline | $rt_prefix ./build/ai-clean/apps/cu_cp/srscucp -c '$cu_cp_config' >/tmp/split_cu_cp_stdout.log 2>&1"
wait_for_sctp_listener "127.0.20.1" "38462" "CU-CP E1AP"
wait_for_sctp_listener "127.0.10.1" "38472" "CU-CP F1AP"

setsid -f bash -c "tail -f /dev/null | $rt_prefix ./build/ai-clean/apps/cu_up/srscuup -c '$cu_up_config' >/tmp/split_cu_up_stdout.log 2>&1"
sleep 2
setsid -f bash -c "tail -f /dev/null | $rt_prefix ./build/ai-clean/apps/du/srsdu -c '$du_config' >/tmp/split_du_stdout.log 2>&1"
sleep 5
